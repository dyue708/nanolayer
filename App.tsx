...
const App: React.FC = () => {
  ...
  const addLayerFromFile = useCallback(async (file: File) => {
      try {
          ...
          if (newWidth > canvasDims.width || newHeight > canvasDims.height) setCanvasDims({ width: newWidth, height: newHeight });
  ...
  const handleDeleteLayer = useCallback((id: string) => {
      setLayers(prev => {
          ...
          if (newLayers.length === 0) setTimeout(() => setCanvasDims({ width: 0, height: 0 }), 0);
          return newLayers;
      });
  ...
  const handleGeminiAction = async (promptText: string) => {
    ...
    try {
      ...
      // New Region Selection Logic: Send Full Image with ROI instructions
      if (activeLayer) {
          base64Img = canvasToBase64(activeLayer.canvas);
          if (selection && selection.width > 5 && selection.height > 5) {
              // Calculate relative ROI coordinates
              ...
              finalPrompt = `Modify only the specific region located at approximately (X:${relX}%, Y:${relY}%) with size (W:${relW}%, H:${relH}%) of the provided image. The modification should be: ${promptText}. CRITICAL: Keep all background details and objects outside this area 100% identical to the original image. Maintain the original image dimensions and overall composition.`;
          }
      }
      ...
      if (aspectRatio || !activeLayer) {
          ...
          // If we have an active layer, ensure the result matches its natural size
          resultCanvas = await base64ToCanvas(newImageBase64, activeLayer.canvas.width, activeLayer.canvas.height);
      }

      // To ensure "维持其他区域不变", we composite the result specifically in the ROI if needed
      // but the user wants it to feel like a "Focus Area" redo.
      // We'll create a new layer for the result.
